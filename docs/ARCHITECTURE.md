...
interaction is used. App restore, panel construction, cached session access, and polling never start
...
session does not repeat the device-code round trip. They are never written to `localStorage`, a URL, a log or
...
cs-control asks Linkspan, the session's main process, for the Jupyter server; Linkspan builds the Python
