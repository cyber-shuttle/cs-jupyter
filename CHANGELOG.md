...
- Sign-in through CILogon's authorization-code flow with PKCE, brokered by cs-plane; a single button, no
...
- Every cs-plane response is validated against a shared `Validator` vocabulary in `Common.ts`; an
